    ...
    <div ref="tableRef">
      <el-form ref="queryRef" :inline="true" :model="queryForm" class="search_left">
        <el-form-item class="el-select-extra" label="" prop="userId">
          <SelectNickname v-model="queryForm.nickname" placeholder="用户昵称" disabled />
        </el-form-item>
...
import { enhanceWithUserInfo, ANONYMOUS_USER_ID } from '@/api/chat'
import router from '@/router'
import { truncateText } from '@/utils/dataFormat'
import { getLiveRoomMessageList } from '@/api/live'
...
const fetchData = async (_queryForm: any = queryForm) => {
  ...
  try {
    const response = await getLiveRoomMessageList(params)
    if (response.code === 0) {
...
watch([roomId, nickname], async (newVal) => {
  console.log(newVal)
  await fetchData()
})
onMounted(async () => {
