  ...
  <div :key="componentKey" id="system_account" class="main_container">
    ...
    <section class="main_flex_table">
      ...
      >
        ...
        <template #bizType="{ scope }">
          {{ getScoreChangeType(scope.row.changeScore) }}
        </template>
      </m-table>
    </section>
  </div>
...
import type { IReqPointRulesDetailData, IReqPointRulesDetailQueryParams } from '@/api/user/pointRules/type'
import { truncateText } from '@/utils/dataFormat'
const modalsStore = useModalsStore()
const valueBus = useValueBus()
const tableData = ref<IReqPointRulesDetailData[]>([])
...
const createTime = computed(() => route.query.createTime || '')
...
const getScoreChangeType = computed(() => (changeScore: number | null | undefined) => {
  if (changeScore === null || changeScore === undefined) return '-'
  const score = Number(changeScore)
  if (isNaN(score)) return '-'
  if (score === 0) return '积分减少'
  return score > 0 ? '积分增加' : '积分减少'
})
watch(
  componentKey,
  (newVal) => {
...
watch(
  ...
  (newValue) => {
    if (newValue) {
      queryForm.nickname = newValue[0]
      queryForm.startTime = newValue[1]
      queryForm.endTime = newValue[1]
      fetchData()
    }
  },
  {
...
const fetchData = async () => {
  let params: IReqPointRulesDetailQueryParams = {
    ...queryForm,
...
onMounted(async () => {
  ...
  if (route.query.createTime) {
    queryForm.startTime = route.query.createTime
    queryForm.endTime = route.query.createTime
  }

  if (route.query.nickname) {
    queryForm.nickname = route.query.nickname
  }
  await fetchData()
})
</script>
