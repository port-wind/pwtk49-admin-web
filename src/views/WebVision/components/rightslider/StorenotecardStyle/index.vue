<template>
  <div class="StorenotecardStyle">
    ...
    <el-form label-width="80px" :model="datas" :rules="rules">
      ...
      <p style="color: #969799; font-size: 12px; margin-top: 10px">鼠标拖拽调整卡片顺序</p>
            ...
            <section class="imgBanner">
              ...
              <div class="imgText">
                ...
                <div class="imgTextChild">
                  <!-- 选择类型 -->
                  <el-select v-model="element.linktype" placeholder="请选择跳转类型" @change="selectType(index)">
                    <el-option
                  ...
                  <!-- 输入外部链接 -->
                  <el-input placeholder="请输入链接，输入前确保可以访问" v-model="element.http.externalLink"></el-input>
                </div>
              </div>
            </section>
          </template>
      ...
      <!-- 上传图片 -->
      <el-button @click="$refs.upload.showUpload()" class="uploadImg" type="primary" plain>点击添加卡片</el-button>
      ...
      <el-form-item class="lef" label="查看更多">
        ...
        <div class="imgText1" v-show="datas.viewMore1 || datas.viewMore2">
          <!-- 选择类型 -->
          <el-select style="width: 60%" v-model="datas.linktype" placeholder="请选择跳转类型">
            <el-option v-for="item in optionsType1" :key="item.name" :label="item.name" :value="item.type"></el-option>
          </el-select>
