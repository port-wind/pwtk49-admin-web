<script setup lang="ts" name="HotRecommendationsStyle">
...
import type { IDatas, ILinkItem } from '@/views/WebVision/components/componentsbiz/HotRecommendations/type'
...
const addLinkItem = () => {
  ...
  datas.value.configParamJson.links.push(newItem)
}
...
const removeLinkItem = (index: number) => {
  ...
  datas.value.configParamJson.links.splice(index, 1)
}
...
const applyPresetConfig = (preset: any) => {
  datas.value.configParamJson.listStyleJSON.itemsPerRow = preset.columnsPerRow
}
</script>
