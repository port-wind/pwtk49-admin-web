<template>
  <div class="ListswitchingStyle">
    ...
    <el-form label-width="80px" :model="datas" :rules="rules">
      <!-- 标题内容 -->
      <el-form-item class="lef" label="选择模板" v-show="datas.commoditylisttype !== 2">
      ...
      <div class="commodityType" v-show="datas.commoditylisttype !== 2">
        ...
        >
          <span
            ...
            :class="[item.type === datas.commodityType ? 'active' : '', item.icon]"
      ...
      <el-form-item label="商品类型" class="lef">
        <el-radio-group v-model="datas.commoditylisttype">
          <el-radio :value="index - 1" v-for="index in 3" :key="index">类型{{ index }}</el-radio>
        </el-radio-group>
      </el-form-item>

      <!-- tabbar颜色 -->
      <el-form-item label="标签颜色" class="lef" v-show="datas.commoditylisttype !== 0">
        <el-color-picker
      ...
      <div v-show="datas.commoditylisttype === 0">
        ...
        <p style="color: #969799; font-size: 12px; margin-top: 10px">鼠标拖拽调整商品顺序</p>

        <!-- 图片广告 -->
        <div v-if="datas.imageList[0]">
          <vuedraggable :list="datas.imageList" item-key="index" :forceFallback="false" :animation="200">
            <template #item="{ element, index }">
              <section class="imgBanner">
                ...
                <div class="imgText">
                  <div>
                    <el-input disabled="disabled" style="width: 65%" v-model="element.name" />
                    <el-input disabled="disabled" type="number" style="width: 35%" v-model.number="element.price" />
                  </div>
        ...
        <!-- 上传图片 -->
        <el-button @click="dialogVisibleshow('imageList', null)" class="uploadImg" type="primary" plain>
          点击添加商品
        </el-button>
      </div>
      ...
      <div v-show="datas.commoditylisttype !== 0">
        <h5 style="color: #000; font-size: 14px; margin-left: 7px">
          添加商品分组
          <el-button style="padding: 2px 4px; margin-left: 200px" @click="addGrouping" type="primary" plain>
            添加
          </el-button>
        </h5>
        <p style="color: #969799; font-size: 12px; margin-left: 7px; margin-top: 10px">鼠标拖拽调整分组顺序</p>

        <!-- 分类名称 -->
        <section v-for="(item, index) in datas.commoditylisttypetab" :key="index">
          <div class="bor" />

          <el-input v-model="item.text" class="tit" style="width: 100px" placeholder="请输入分组名称" />
          <van-icon
            name="delete-o"
            style="cursor: pointer; padding: 2px 4px; margin-left: 200px; color: red"
            @click="delecommoditylisttypetab(index)"
          />

          <vuedraggable :list="item.imageList" item-key="index" :forceFallback="false" :animation="200">
          ...
          <el-button @click="dialogVisibleshow('commoditylisttypetab', index)" class="uploadImg" type="primary" plain>
            点击添加商品
          </el-button>
        </section>
      </div>
      ...
      <el-form-item class="lef" label="背景图片">
        <div class="shop-head-pic" style="text-align: center">
          ...
          <div class="shop-head-pic-btn" style="text-align: center">
            <el-button @click="showUpload('0')" class="uploadImg" type="primary" plain>更换图片</el-button>
      ...
      <el-form-item class="lef" label="商品价格">
        ...
        <el-checkbox style="margin-left: 196px" v-model="datas.priceofcommodity" />
      </el-form-item>
      ...
      <el-form-item class="lef" label="购买按钮">
        ...
        <el-checkbox style="margin-left: 196px" v-model="datas.purchasebutton" />
      </el-form-item>

      <el-radio-group v-model="datas.purchasebuttonType" class="radi1" v-show="datas.purchasebutton">
        <el-radio :value="index - 1" v-for="index in 8" :key="index">样式{{ index }}</el-radio>

        <el-input v-show="datas.purchasebuttonType > 3" style="width: 40%; margin-top: 10px" v-model="datas.purchase" />
      </el-radio-group>
      ...
      <el-form-item class="lef" label="商品角标">
        ...
        <el-checkbox style="margin-left: 196px" v-model="datas.commoditycorner" />
      </el-form-item>

      <el-radio-group v-model="datas.commoditycornertype" class="radi1" v-show="datas.commoditycorner">
        <el-radio :label="index" v-for="(item, index) in marker" :key="index">{{ item }}</el-radio>
      </el-radio-group>
      ...
      <el-form-item class="lef" label="位置">
        <el-radio-group v-model="datas.tagPosition" class="radi1">
          <el-radio :value="index" v-for="(item, index) in tagPosition" :key="index">{{ item }}</el-radio>
        </el-radio-group>
      </el-form-item>
    </el-form>
