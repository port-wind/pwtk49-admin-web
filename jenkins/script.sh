...
# ===== 同步到 197 服务器 =====
log_message "=============== 开始同步到 197 服务器 ==============="
send_telegram_message "📤 开始同步到 197 服务器"

# 切换回项目目录
cd /home/www/websites/frontend/pwtk-admin-web || {
    log_message "错误: 无法切换到项目目录"
    send_telegram_message "❌ 同步失败: 无法切换到项目目录"
    exit 1
}

# 同步 dist-dev 文件到 197 服务器
log_message "同步 dist-dev 到 197 服务器"
if ! scp -i ~/.ssh/id_rsa_for197 -r dist-dev/* root@192.168.0.197:/usr/share/nginx/html/admin 2>&1 | tee -a "$BUILD_LOG_FILE"; then
    log_message "错误: 同步 dist-dev 到 197 服务器失败"
    send_telegram_message "❌ 同步失败: dist-dev 文件同步到 197 服务器失败"
    exit 1
fi
log_message "dist-dev 同步成功"

# 同步 webConfigApi.js 文件到 197 服务器
log_message "同步 49gallery/webConfigApi.js 到 197 服务器"
if ! scp -i ~/.ssh/id_rsa_for197 49gallery/webConfigApi.js root@192.168.0.197:/usr/share/nginx/html/admin/ 2>&1 | tee -a "$BUILD_LOG_FILE"; then
    log_message "错误: 同步 webConfigApi.js 到 197 服务器失败"
    send_telegram_message "❌ 同步失败: webConfigApi.js 文件同步到 197 服务器失败"
    exit 1
fi
log_message "webConfigApi.js 同步成功"

send_telegram_message "✅ 文件同步到 197 服务器完成"
log_message "=============== 所有文件已成功同步到 197 服务器 ==============="